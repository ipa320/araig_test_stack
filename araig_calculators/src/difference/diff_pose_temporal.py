    ...
    def calculate(self):
        ...
        if temp[self._sub_topic_start] is not None:
            if self._prestate_start is False and \
                ...
                if temp[self._sub_topic_pose] is None:
                    rospy.logwarn(
                        rospy.get_name() +
                        ": Got Start Signal but didn't get object pose, please resent start signal")
                    temp[self._sub_topic_start].data = False
                else:
                    self._posestamp_start = temp[self._sub_topic_pose]
                    rospy.loginfo(rospy.get_name() + ": Started")
                    self.log_msg['Started'] = float(
                        self._posestamp_start.header.stamp.secs +
                        float(
                            self._posestamp_start.header.stamp.nsecs *
                            (1e-9)))
        ...
        if temp[self._sub_topic_stop] is not None and \
            ...
            if self._prestate_stop is False and \
                ...
                self.log_msg['Stopped'] = float(
                    self._posestamp_stop.header.stamp.secs +
                    float(
                        self._posestamp_stop.header.stamp.nsecs *
                        (1e-9)))

                delta_x = abs(
                    self._posestamp_start.pose.position.x -
                    self._posestamp_stop.pose.position.x)
                delta_y = abs(
                    self._posestamp_start.pose.position.y -
                    self._posestamp_stop.pose.position.y)

                pub_msg_angular.data = abs(
                    self.get_yaw_from_quaternion(
                        self._posestamp_start.pose.orientation) -
                    self.get_yaw_from_quaternion(
                        self._posestamp_stop.pose.orientation))
                pub_msg_angular.header.stamp = rospy.Time.now()

                pub_msg_position.data = math.sqrt(
                    (delta_x * delta_x) + (delta_y * delta_y))
                pub_msg_position.header.stamp = rospy.Time.now()

                self.PubDiag[self._pub_topic_angular].publish(pub_msg_angular)
                self.PubDiag[self._pub_topic_position].publish(
                    pub_msg_position)
                rospy.loginfo(
                    "{}: Delta position is {}".format(
                        rospy.get_name(),
                        pub_msg_position.data))
                self.log_msg['Delta_position'] = {}
                self.log_msg['Delta_position']['timestamp'] = float(
                    pub_msg_position.header.stamp.secs +
                    float(
                        pub_msg_position.header.stamp.nsecs *
                        (1e-9)))
