...
class diffPosesSpatial(BaseCalculator):
    ...
    def calculate(self):
        ...
        if flag_test_ready:
            delta_x = abs(temp[self._sub_topic_object_1].pose.position.x -
                          temp[self._sub_topic_object_2].pose.position.x)
            delta_y = abs(temp[self._sub_topic_object_1].pose.position.y -
                          temp[self._sub_topic_object_2].pose.position.y)
            ...
            if temp[self._sub_topic_singal].data:
                if(self.pub_only_state_change(pre_state=self.pre_signal_state_angular,
                                              ...
                                              log="{}: Delta angle is {}".format(rospy.get_name(),
                                                                                 pub_msg_angular.data))):
                    self.pre_signal_state_angular = temp[self._sub_topic_singal].data
                    self.log_msg["Delta_angle"] = {}
                    self.log_msg["Delta_angle"]["timestamp"] = float(
                        pub_msg_angular.header.stamp.secs +
                        float(
                            pub_msg_angular.header.stamp.nsecs *
                            (1e-9)))
                ...
                if(self.pub_only_state_change(pre_state=self.pre_signal_state_position,
                                              ...
                                              log="{}: Delta position is {}".format(rospy.get_name(),
