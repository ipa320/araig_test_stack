    ...
    if os.path.isdir(folder) is False:
        if create is True:
            try:
                os.makedirs(folder)
                rospy.logwarn(
                    "{}: Folder {} is not exist, created it.".format(
                        rospy.get_name(), folder))
                return True
            except OSError:
                rospy.logerr(
                    "{}: Folder {} is not exist, But can't create it, Please check if you have right to create.".format(
                        rospy.get_name(), folder))
                return False
        else:
            rospy.logerr(
                "{}: Folder {} is not exist.".format(
                    rospy.get_name(), folder))
    return True
...
def create_logging_folder(folder):
    if check_folder(folder, True):
        ...
        rospy.logerr(
            "{}: Can't find and create folder {}, Please create it manually.".format(
                rospy.get_name(), folder))
        sys.exit()
    ...
    try:
        ...
        rospy.loginfo(
            "{}: Successfully created the directory {}".format(
                rospy.get_name(), folder_name))
    except OSError as err:
        rospy.logerr(
            "{}: Failed to create {}, error msg: {}".format(
                rospy.get_name(),
                folder_name,
                str(err)))
        sys.exit()
...
def get_sub_folder():
    ...
    if check_folder(root):
        ...
        rospy.logerr(
            "{}: Can't find folder {}, Please create it manually.".format(
                rospy.get_name(), root))
...
def create_file(folder, filename):
    with open(folder + "/" + filename, mode='a'):
        pass
