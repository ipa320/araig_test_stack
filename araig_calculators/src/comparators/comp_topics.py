...
class compTopics(BaseCalculator):
    ...
    def __init__(self,
        ...
        if sub_dict is None or len(sub_dict) < 2:
            rospy.logerr(
                "{}:  Please provide two topics to subscribe".format(
                    rospy.get_name()))
    ...
    def calculate(self):
        ...
        for topic in self.SubDict.keys():
            ...
            if temp[topic] is None:
                flag_test_ready = False

        if flag_test_ready:
            msg.header.stamp = rospy.Time.now()
            if temp[self._sub_topic_1].data and \
                    temp[self._sub_topic_2].data:
                msg.data = True
            else:
                msg.data = False
