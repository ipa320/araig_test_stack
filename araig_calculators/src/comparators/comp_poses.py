...
class compPoses(BaseCalculator):
    ...
    def calculate(self):
        ...
        if flag_test_ready:
            delta_x = abs(temp[self._sub_topic_pose_1].pose.position.x -
                          temp[self._sub_topic_pose_2].pose.position.x)
            delta_y = abs(temp[self._sub_topic_pose_1].pose.position.y -
                          temp[self._sub_topic_pose_2].pose.position.y)
            ...
            if self._delta_theta_thresh_degrees is None or delta_theta <= self._delta_theta_thresh_degrees:
                flag_theta = True
            else:
                flag_theta = False

            if flag_theta and flag_pose:
                pub_msg.data = True
            else:
                pub_msg.data = False

            if self.pub_only_state_change(
