...
class TestCalcPoseDelta(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pub_signal = rospy.Publisher(
            _pub_signal, BoolStamped, latch=True, queue_size=10)
    ...
    def test_expected(self):
        ...
        self.assertEqual(
            self.delta_pose,
            5.0,
            'test1: {} is not equal to 5.0'.format(
                self.delta_pose))
        self.assertEqual(
            self.delta_angle,
            90.0,
        ...
        self.assertEqual(
            self.msg_seq_angle,
            1,
            'test1: msg published {} times'.format(
                self.msg_seq_angle))
        self.assertEqual(
            self.msg_seq_pose,
            1,
        ...
        self.assertEqual(
            self.msg_seq_angle,
            1,
            'test2: msg published {} times'.format(
                self.msg_seq_angle))
        self.assertEqual(
            self.msg_seq_pose,
            1,
        ...
        self.assertEqual(
            self.msg_seq_angle,
            2,
            'test3: msg published {} times'.format(
                self.msg_seq_angle))
        self.assertEqual(
            self.msg_seq_pose,
            2,
