...
class TestVelocityInterpreter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.max_vel = rospy.get_param(
            "/interpreters/velocity_interpreter/max_vel")
        self.vel = None
    ...
    def test_expected(self):
        ...
        self.assertAlmostEqual(
            self.transfer_timestamp(
                pub_msg_start.header.stamp), self.transfer_timestamp(
                self.time), msg='test1: Send signal at {}, send velocity at {}'.format(
                self.transfer_timestamp(
                    pub_msg_start.header.stamp), self.transfer_timestamp(
                        self.time)), delta=0.5)
        ...
        self.assertAlmostEqual(
            self.transfer_timestamp(
                pub_msg_stop.header.stamp), self.transfer_timestamp(
                self.time), msg='test2: Send signal at {}, send velocity at {}'.format(
                self.transfer_timestamp(
                    pub_msg_stop.header.stamp), self.transfer_timestamp(
                        self.time)), delta=0.5)
        ...
        self.assertAlmostEqual(
            self.transfer_timestamp(
                pub_msg_start.header.stamp), self.transfer_timestamp(
                self.time), msg='test4: Send signal at {}, send velocity at {}'.format(
                self.transfer_timestamp(
                    pub_msg_start.header.stamp), self.transfer_timestamp(
                        self.time)), delta=0.5)
