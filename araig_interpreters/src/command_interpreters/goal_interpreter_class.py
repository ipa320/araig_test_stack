...
class GoalInterpreterClass():
    ...
    def __init__(self,
        ...
        if self.action_name != "None":
            rospy.loginfo(
                rospy.get_name() +
                ": will try to action server: {}...".format(
                    self.action_name))
            try_times = 0
            while self.action_init(wait_for_action) is False and try_times < 5:
                rospy.logwarn(
                    rospy.get_name() +
                    ": cannot connected to action server: {}, will try again".format(
                        self.action_name))
                try_times += 1
                rospy.sleep(1)
            if try_times == 5:
                rospy.logerr(
                    rospy.get_name() +
                    ": cannot connected to action server: {}, shutdown this node".format(
                        self.action_name))
                rospy.signal_shutdown("cannot connected to action server")
        else:
            rospy.logwarn(
                rospy.get_name() +
                ": will not connect to action server")
        self.pub_init()
    ...
    def action_init(self, wait_time):
        ...
        if self.action_client.wait_for_server(rospy.Duration(wait_time)):
            rospy.loginfo(
                rospy.get_name() +
                ": connected to action server: {}...".format(
                    self.action_name))
    ...
    def sub_init(self, topic, datatype):
        self.sub_diag = {}
        self.sub_diag[topic] = rospy.Subscriber(topic, datatype, getattr(
            self, "callback_" + topic.replace('/', '')), (topic))
    ...
    def call_action(self):
        ...
        if not wait:
            rospy.logerr(
                rospy.get_name() +
                ": didn't get response from {}".format(
                    self.action_name))
            return False
        else:
            ...
            if status == 3:
                ...
                rospy.logwarn(
                    rospy.get_name() +
                    ": move base failed to move to the goal")
    ...
    def read_action_result(self):
        with GoalInterpreterClass.LOCK[self.sub_topic_result]:
            ...
            rospy.logwarn(rospy.get_name() + ": result: " +
                          str(self.temp_sub[self.sub_topic_result]))
    ...
    def main(self):
        ...
        try:
            ...
            while not rospy.is_shutdown():
                ...
                if pre_signal is False and self.temp_sub[self.sub_topic] is True:
                    self.pub_result(False)
                    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                        loop.run_until_complete(
                            self.main_pub_and_wait_action(loop, pool))
