...
class VelocityInterpreterClass():
    ...
    def sub_init(self):
        sub_diag = {}
        sub_diag[self.sub_topic_start] = rospy.Subscriber(
            self.sub_topic_start, BoolStamped, self.callback_start, (self.sub_topic_start))
        sub_diag[self.sub_topic_stop] = rospy.Subscriber(
            self.sub_topic_stop, BoolStamped, self.callback_start, (self.sub_topic_stop))
    ...
    def main(self):
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if temp[self.sub_topic_start] == 1 and temp[self.sub_topic_stop] == 1 and flag_both_high is False:
                    rospy.logwarn(
                        rospy.get_name() +
                        ": Both  START and STOP signals are high")
                    flag_both_high = True

                if pre_stop and pre_stop and temp[self.sub_topic_start] == 0 and temp[self.sub_topic_stop] == 0:
                    rospy.logwarn(
                        rospy.get_name() +
                        ": reset velocity interpreter")
                    pre_start = False
                    pre_stop = False
